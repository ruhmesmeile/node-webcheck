  ...
  return function followHTMLMiddleware(ro, cb){
    ...
    if(/text\/x?-?html|application\/x?-?html/.test(result.headers['content-type'])) { // && result.window.$
      if(!result || !result.window || !result.window.$) {
        ro.addReport("error", "followHTML", "HTML is invalid!");
        return cb();
      }
      var $ = result.window.$;
      ...
      //----------------------------------
      var base = $("base").attr("href") || ro.getURL();
      ...
      $("a[href]").each(function(index,a) {
        if (a.getAttribute("href")) {
          var href = resolveUrl(base, a.getAttribute("href"));
          ...
          if(opts.filter.test(href) && !opts.ignore.test(href)) {
            ro.follow(href);
          }
      ...
      $("script[src]").each(function(index, script){
        if (script.getAttribute("src")) {
          var href = resolveUrl(base, script.getAttribute("src"));
          var target, obj;
          if(target = wc.getResource(href)) {
          ...
          if(opts.filter.test(href) && !opts.ignore.test(href)) ro.follow(href);
          else ro.addResource(href);
        }
      });
      $('link[rel="stylesheet"]').each(function(index, link){
        if (link.getAttribute("href")) {
          var href = resolveUrl(base, link.getAttribute("href"));
          var target, obj;
          if(target = wc.getResource(href)) {
      ...
      $('img[href]').each(function(index, img){
        if (img.getAttribute("src")) {
          var href = resolveUrl(base, img.getAttribute("src"));
          var target, obj;
          if(target = wc.getResource(href)) {
